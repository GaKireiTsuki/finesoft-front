...
返回的源码调用 `registerProxyRoutes(app, config)`。自定义生成入口需要从 `@finesoft/front/ssr` 导入 `registerProxyRoutes` 并提供 Hono `app`；标准适配器自动完成这些绑定。开发环境和生成主机共用路径校验、二进制响应与大小限制实现。它属于构建产物，不是应用启动 API。密钥值配置在主机上，路由声明变化后重新构建。`auth.envKey` 在可用时读取 `process.env`；没有 `process` 的主机可在运行时注册配置中提供 Authorization 请求头。平台包体限制及部署规则以该平台当前文档为准。
