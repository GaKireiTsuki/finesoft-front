...
export interface NavigationDefinition {
     ...
     * `initial` 是工厂时，对 `url`（由浏览器 host 显式提供）求值；返回 `undefined`
 ...
 *
 * // CSR
 * startBrowserApp({ bootstrap, mount, callbacks, navigation: nav.toBrowserConfig(window.location.pathname + window.location.search) });
 * // SSR
...
export function defineNavigation(options: DefineNavigationOptions): NavigationDefinition {
    ...
    return {
        ...
        toBrowserConfig(url: string): NavigationBrowserConfig {
            return {
                initial: resolveInitialTree(initial, url),
                codec,
                beforeLoad,
                afterLoad,
                getErrorPage,
