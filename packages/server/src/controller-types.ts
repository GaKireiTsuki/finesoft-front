...
export function assertEditablePath(root: string, file: string): void {
    let existing = file;
...
function generateTypes(
    ...
    const serverBases = new Set<ts.Symbol>();
    ...
    const serverController = (node: ts.ClassDeclaration) => {
    ...
    const printType = (type: ts.Type): string => {
        if (!type || type.flags & (compiler.TypeFlags.Any | compiler.TypeFlags.Unknown))
            throw Error(
                "Cannot infer controller input. Check route schemas and @finesoft/front types.",
            );
    ...
    if (managed.size || fs.existsSync(output)) {
        ...
        updates.set(
            ...
            `${GENERATED}export interface ControllerInputs {\n${[...managed.values()]
                ...
                .map(
                    (entry) =>
                        `export type ${exportedName(entry)} = ControllerInputs[${JSON.stringify(entry.key)}] & { readonly context: ${serverController(entry.node) ? 'import("@finesoft/front").ServerControllerContext' : 'import("@finesoft/front").ControllerContext'} };\nexport type ${exportedName(entry, "Failure")} = ${exportedName(entry)} & { readonly error: Error };`,
                )
                .join("\n")}\n`,
        );
    }
