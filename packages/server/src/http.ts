...
export function createHttpHandler(
    input: HttpHandlerOptions | (() => HttpHandlerOptions),
): HttpHandler {
    function prepare(options: HttpHandlerOptions) {
            ...
            if (
                routes.some(
                    (other) =>
                        ...
                        other.compiledPath.descriptor.shape ===
                            endpoint.compiledPath.descriptor.shape,
                )
            )
                throw new ExecutionError("configuration");
        }
    ...
    // Worker module evaluation cannot initialize runtimes that use random IDs or I/O.
    // Prepare once in the first invocation; ordinary options keep eager validation.
    let prepared = typeof input === "function" ? undefined : prepare(input);
    return {
        async fetch(request, bindings = {}, host) {
            const { options, routes } = (prepared ??= prepare(
            ...
            for (const candidate of sameMethod) {
                ...
                for (const parameter of candidate.endpoint.compiledPath.descriptor.parameters) {
                    ...
                    try {
                        result = await codec["~standard"].validate(
                            candidate.params[parameter.name],
                        );
                    } catch {
                        valid = false;
                        break;
            ...
            try {
                ...
                const ownedResponse = await ownResponse(
                    response,
                    execution,
                    abort,
                    signal,
