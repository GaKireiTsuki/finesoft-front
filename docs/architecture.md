...
Operation 归一化后直接选择实现并调用 `handler(input, context)`。宿主 override、输入输出校验、策略和取消仍在同一执行链中。类式业务可选 `BaseController.execute(input, context)` 与 `fallback`，不再重复声明 intentId，也没有 IntentDispatcher 的第二份注册表。
